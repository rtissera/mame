...
// copyright-holders:Romain Tiserand
/***************************************************************************
...
#ifndef MAME_DEVICES_CPU_DRCBEA64_H
#define MAME_DEVICES_CPU_DRCBEA64_H
...
class drcbe_a64 : public drcbe_interface
{
	typedef uint32_t (*a64_entry_point_func)(uint8_t *rbpvalue, a64code *entry);

public:
	// construction/destruction
	drcbe_a64(drcuml_state &drcuml, device_t &device, drc_cache &cache, uint32_t flags, int modes, int addrbits, int ignorebits);
	virtual ~drcbe_a64();
...
private:
	...
	public:
		...
		be_parameter(drcbe_a64 &drcbe, const uml::parameter &param, uint32_t allowed);
	...
	void smart_call_r64(Assembler &a, a64code *target, Gp const &reg);
	void smart_call_m64(Assembler &a, a64code **target);
	...
	a64_entry_point_func    m_entry;                // entry point
	a64code *               m_exit;                 // exit point
	a64code *               m_nocode;               // nocode handler

	// state to live in the near cache
	struct near_state
	{
		a64code *           debug_cpu_instruction_hook;// debugger callback
		a64code *           debug_log_hashjmp;      // hashjmp debugging
		a64code *           debug_log_hashjmp_fail; // hashjmp debugging
		a64code *           drcmap_get_value;       // map lookup helper
	...
	// globals
	typedef void (drcbe_a64::*opcode_generate_func)(Assembler &a, const uml::instruction &inst);
...
#endif /* MAME_DEVICES_CPU_DRCBEA64_H */
